...
local keys = {
   ...
   -- copy/paste --
   {
      ...
      if is_selection_active then
         -- Copy to clipboard if text is selected
         window:perform_action(act.CopyTo 'ClipboardAndPrimarySelection', pane)
      else
         -- Send Ctrl+C to stop processes if no text is selected
   ...
   -- panes: layouts --
   -- dev layout: full-height left pane, two stacked on the right
   {
      key = 'd',
      mods = mod.SUPER_REV,
      action = wezterm.action_callback(function(window, pane)
         local cwd = pane:get_current_working_dir()
         local is_wsl = cwd and cwd.host and #cwd.host > 0
         local dev_dir = is_wsl and '~/dev' or 'C:\\Users\\ivann\\dev'
